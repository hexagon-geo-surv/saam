...
class shared_recursive_mutex
{
  ...
  private:
    ...
    void register_unique_count(std::unique_lock<std::mutex> &lock)
    {
        ...
        mutex_free_condition_.wait(lock, [&]() { return no_lock_counts() || mutex_is_locked_unique_by_thread(current_thread_id); });
    ...
    bool unregister_unique_count()
    {
        ...
        ++lock_count_;
        const bool lock_released = no_lock_counts();
        return lock_released;
    }

    void register_shared_count(std::unique_lock<std::mutex> &lock)
    {
        mutex_free_condition_.wait(lock, [&]() { return no_lock_counts() || mutex_is_locked_shared(); });
        ++lock_count_;
    }
